...
export function buildDial() {
    ...
    const lx = cx - mr, rx = cx + mr;
    dialSvgNode.appendChild(svgNode('path', {
    ...
    const p1 = degToPoint(180, r), p5 = degToPoint(0, r);
    dialSvgNode.appendChild(svgNode('path', {
        d: `M ${p1.x} ${p1.y} A ${r} ${r} 0 0 1 ${p5.x} ${p5.y}`,
        fill: 'none', stroke: 'black', 'stroke-width': '2.6'
    }));
    dialSvgNode.appendChild(svgNode('line', {
        x1: p1.x, y1: p1.y, x2: p5.x, y2: p5.y,
        stroke: 'black', 'stroke-width': '2.0'
    }));
    dialSvgNode.appendChild(svgNode('path', {
    ...
    const PETAL_WIDTH = PETAL_LEN * 0.30;
    ...
    for (let f = 1; f <= TOTAL_FLOORS; f++) {
        ...
        gPetals.appendChild(svgNode('path', {
            d: `M ${cx} ${cy}
    ...
    needleGroup.appendChild(svgNode('line', {
        x1: cx, y1: cy + 3, x2: cx, y2: cy - needleLen,
        stroke: '#fdc279', 'stroke-width': '3.0', 'stroke-linecap': 'round'
    }));
    dialSvgNode.appendChild(needleGroup);

    dialSvgNode.appendChild(svgNode('circle', {
        cx: cx, cy: cy, r: '4.5',
        fill: 'black', stroke: '#fdc279', 'stroke-width': '1.2'
    }));
...
export function setDialNeedle(deg) {
    ...
    const isClose = (f) =>
        Math.abs(floorToDeg(f) - deg) < (LABEL_STEP_DEG / 2);
